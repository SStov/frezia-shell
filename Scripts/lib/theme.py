...
def _create_theme_dict(primary, secondary, tertiary, error, surface, surface_variant, on_surface, on_surface_variant, outline, outline_variant, mode):
    from .color import adjust_surface
    from .contrast import ensure_contrast
    from .color import Color
    
    p_c_l = 0.2 if mode == "dark" else 0.9
    s_c_l = 0.2 if mode == "dark" else 0.9
    t_c_l = 0.2 if mode == "dark" else 0.9
    e_c_l = 0.2 if mode == "dark" else 0.9
    ...
    return {
        ...
        "on_primary": ensure_contrast(on_surface, primary, 4.5).to_hex(),
        "primary_container": adjust_surface(primary, primary.to_hsl()[1], p_c_l).to_hex(),
        "on_primary_container": primary.to_hex(),
        "secondary": secondary.to_hex(),
        "on_secondary": ensure_contrast(on_surface, secondary, 4.5).to_hex(),
        "secondary_container": adjust_surface(secondary, secondary.to_hsl()[1], s_c_l).to_hex(),
        "on_secondary_container": secondary.to_hex(),
        "tertiary": tertiary.to_hex(),
        "on_tertiary": ensure_contrast(on_surface, tertiary, 4.5).to_hex(),
        "tertiary_container": adjust_surface(tertiary, tertiary.to_hsl()[1], t_c_l).to_hex(),
        "on_tertiary_container": tertiary.to_hex(),
        "error": error.to_hex(),
        "on_error": ensure_contrast(on_surface, error, 4.5).to_hex(),
        "error_container": adjust_surface(error, error.to_hsl()[1], e_c_l).to_hex(),
        "on_error_container": error.to_hex(),
        ...
        "background": surface.to_hex(),
        "on_background": on_surface.to_hex(),
        
        "primary_fixed": primary.to_hex(),
        "primary_fixed_dim": primary.to_hex(),
        "on_primary_fixed": on_surface.to_hex(),
        "on_primary_fixed_variant": on_surface.to_hex(),
        "secondary_fixed": secondary.to_hex(),
        "secondary_fixed_dim": secondary.to_hex(),
        "on_secondary_fixed": on_surface.to_hex(),
        "on_secondary_fixed_variant": on_surface.to_hex(),
        "tertiary_fixed": tertiary.to_hex(),
        "tertiary_fixed_dim": tertiary.to_hex(),
        "on_tertiary_fixed": on_surface.to_hex(),
        "on_tertiary_fixed_variant": on_surface.to_hex(),
        
        "surface_container_lowest": adjust_surface(surface, surface.to_hsl()[1], sc_lowest).to_hex(),
        "surface_container_low": adjust_surface(surface, surface.to_hsl()[1], sc_low).to_hex(),
        "surface_container": adjust_surface(surface, surface.to_hsl()[1], sc_normal).to_hex(),
        "surface_container_high": adjust_surface(surface, surface.to_hsl()[1], sc_high).to_hex(),
        "surface_container_highest": adjust_surface(surface, surface.to_hsl()[1], sc_highest).to_hex(),
        
        "shadow": "#000000",
        "scrim": "#000000",
        "surface_dim": surface.to_hex(),
        "surface_bright": surface.to_hex(),
        "inverse_surface": on_surface.to_hex(),
        "inverse_on_surface": surface.to_hex(),
        "inverse_primary": primary.to_hex()
    }


def generate_pastel(palette, mode):
    from .color import Color, shift_hue
    from .palette import find_error_color
...
def generate_vibrant(palette, mode):
    from .color import Color, shift_hue
    from .palette import find_error_color
    ...
    h_s, s_s, l_s = secondary.to_hsl()
    h_t, s_t, l_t = tertiary.to_hsl()
    
    if mode == "dark":
        p_adj = Color.from_hsl(h_p, max(s_p, 0.6), max(l_p, 0.65))
...
def generate_intensified(palette, mode):
    from .color import Color, shift_hue
    from .palette import find_error_color
...
def generate_theme(
    palette: list[Color],
    mode: ThemeMode,
    scheme_type: str = "vibrant"
) -> dict[str, str]:
    if scheme_type == "pastel":
        return generate_pastel(palette, mode)
    elif scheme_type == "intensified":
        return generate_intensified(palette, mode)
    else:
        return generate_vibrant(palette, mode)
